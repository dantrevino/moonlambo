...
function fetchPrices(symbols, fiat, lang) {
  ...
  axios.get(url).then(function(res){
    ...
    for( var i = 0; i< coinArray.length; i++ ){
      ...
      var arrName = selectData.find(function(data){
        return data.id === arrSymbol
      }).text
      var arrLink = 'https://www.coinmarketcap.com/currencies/' + arrName
      ...
      renderArr.push({"url": arrLink, "symbol":arrSymbol,"name": arrName,"price":arrPrice,"cssClass": cssCls, "changePct": arrChgPct})
    }
    loadTable(renderArr)
  }).catch(function(err){
    console.log(err)
